	...
	public static class Handler implements ContentHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes pAttrs) throws SAXException {
			...
			switch (localName) {
			  case "icm-plugins":
				if (level != 0) {
					throw error("Element rcm-plugins unexpected at level " + level);
				}
				break;
			  case "icm-plugin":
				  if (level != 1) {
					  throw error("Element rcm-plugin unexpected at level " + level);
				  }
				  final String className = pAttrs.getValue(XMLConstants.NULL_NS_URI, "class");
				  if (className == null  ||  className.length() == 0) {
					  throw error("Missing, or empty attribute: rcm-plugin/@class");
				  }
